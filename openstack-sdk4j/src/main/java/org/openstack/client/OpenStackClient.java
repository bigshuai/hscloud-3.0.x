...
public class OpenStackClient {
	...
	// private LoggingFilter loggingFilter = new
	// LoggingFilter(Logger.getLogger(OpenStackClient.class.getPackage().getName()),false);

	// private LoggingFilter loggingEntityFilter = new
	// LoggingFilter(Logger.getLogger(OpenStackClient.class.getPackage().getName()),true);
	...
	public static OpenStackClient authenticate(Properties properties,
		...
		client.authAsAdministratorFilter = new XAuthTokenFilter(
				properties.getProperty("identity.admin.token"));
		return client;
	}
	...
	public static OpenStackClient authenticate(Properties properties) {
		...
		Access access = client.getIdentityEndpoint().tokens()
				.post(authentication);
	...
	public static OpenStackClient authenticate() {
		try {
			...
			properties.load(OpenStackClient.class
					.getResourceAsStream("/openstack.properties"));
			return authenticate(properties);
		} catch (IOException e) {
			throw new OpenstackException(
					"openstack.properties not found in the CLASSPATH", e);
		}
	}
	...
	public void exchangeTokenForTenant(String tenantId) {
		...
		Authentication authentication = new KeystoneAuthentication()
				.withTokenAndTenant(access.getToken().getId(), tenantId);
		this.access = target(endpoint, IdentityPublicEndpoint.class).tokens()
				.post(authentication);
	...
	public IdentityPublicEndpoint getIdentityEndpoint() {
		...
		Preconditions.checkNotNull(url,
				"'identity.endpoint.publicURL' property not found");
	...
	public IdentityInternalEndpoint getIdentityInternalEndpoint() {
		...
		Preconditions.checkNotNull(url,
				"'identity.endpoint.internalURL' property not found");
	...
	public IdentityAdministrationEndpoint getIdentityAdministationEndpoint() {
		...
		Preconditions.checkNotNull(url,
				"'identity.endpoint.adminURL' property not found");
	...
	public TenantResource getComputeEndpoint() {
		String region = properties.getProperty("compute.endpoint.default");
		return target(access.getEndpoint("compute", region).getPublicURL(),
				TenantResource.class);
	}

	public TenantResource getComputeEndpoint(String region) {
		try {
			return target(access.getEndpoint("compute", region).getPublicURL(),
					TenantResource.class);
		} catch (Exception e) {
			return getComputeEndpoint();
		}
	}
	...
	public TenantResource getComputeInternalEndpoint() {
		return target(access.getEndpoint("compute", null).getInternalURL(),
				TenantResource.class);
	}
	...
	public TenantResource getComputeAdministationEndpoint() {
		return target(access.getEndpoint("compute", null).getAdminURL(),
				TenantResource.class);
	}
	...
	public ImagesResource getImagesEndpoint() {
		return target(
				access.getEndpoint("image", null).getPublicURL()
						.concat("/images"), ImagesResource.class);
	}

	public ImagesResource getImagesInternalEndpoint() {
		return target(
				access.getEndpoint("image", null).getAdminURL()
						.concat("/images"), ImagesResource.class);
	}
	...
	public AccountResource getStorageEndpoint() {
		return target(access.getEndpoint("object-store", null).getPublicURL(),
				AccountResource.class);
	}

	public AccountResource getStorageInternalEndpoint() {
		return target(access.getEndpoint("object-store", null).getInternalURL()
				.replace("AUTH_", ""), AccountResource.class);
	}

	public AccountResource getStorageAdministationEndpoint() {
		return target(access.getEndpoint("object-store", null).getAdminURL()
				.replace("AUTH_", ""), AccountResource.class);
	}

	public HscloudResource getHscloudEndpoint() {
		return target(access.getEndpoint("ext", null).getPublicURL(),
				HscloudResource.class);
	}
	...
	private <T extends Resource> T target(String absoluteURL, Class<T> clazz,
			boolean useAdministrationToken) {
		try {
			Target target = RestClient.INSTANCE.getJerseyClient().target(
					absoluteURL);
			if (access != null) {
				target.configuration().register(
						useAdministrationToken ? authAsAdministratorFilter
								: authFilter);
			}
			return clazz.getConstructor(Target.class, Properties.class)
					.newInstance(target, properties);
		} catch (Exception e) {
