...
//				adjustVMForm.getForm().reset();
//				createVMWin.remove(adjustVMForm);
//				createVMWin.hide();
			}
		}]
	},{
...
//					alert("aa"+vmIPAdjustField.getValue());
					var adjust = Ext.Ajax.request({
						...
						params : {
							'id' : vmId,
							'newIP' : vmIPAdjustField.getValue()
							},
							success : function(form, action) {
								v_mask.hide();
								var obj = Ext.decode(form.responseText);
...
var configAdjustPanel = Ext.create('Ext.form.Panel', {
	...
	items : [ {
		...
		items : [ {
			xtype : 'label',
			width : 22
		}, vmAdjustAddDiskButton, {
			xtype : 'displayfield',
			margin :'0 10 0 10',
			width : 200,
			value : i18n._('Add a new disk')//添加一块扩展硬盘
		} ]
	} ],
	buttons : [ {
		text : i18n._('OK'),
		handler : function() {
			...
			var vcpusId = vmAdjustCPUComb.getValue();
			var vcpus = vmAdjustCPUStore.getAt(vmAdjustCPUStore.indexOfId(vmAdjustCPUComb.getValue())).get('coreNum');//vmAdjustCPUComb.getValue();
			var vcpusType = vmAdjustCPUStore.getAt(vmAdjustCPUStore.indexOfId(vmAdjustCPUComb.getValue())).get('model');
			var ram = vmAdjustRamStore.getAt(RAMAdjustField.getValue()).get('size');			
			var disk = vmAdjustDiskStore.getAt(DiskAdjustField.getValue()).get('capacity');
			//alert('disk'+disk);
			//alert('ram'+ram);
			var addDisk ='';
			if (vmAdjustDiskArray.length > 0) {
				for ( var i = 0; i < vmAdjustDiskArray.length; i++) {							
					addDisk = addDisk
							+ vmAdjustExtDiskStore.getAt(Ext.getCmp('diskAdjustField' + vmAdjustDiskArray[i]).getValue()).get('capacity')+',';
				}
			}
			if((vmAdjustCPUCoreField.getValue() == vcpus) && 
					(vmAdjustRamField.getValue() == ram) && 
					(vmAdjustDiskArray.length > 0)){//cpu、ram均不变时只传扩展盘
				attachVolumeAction(v_mask,vmid,addDisk);
			}
//			else if(vmAdjustRamField.getValue()==ram){//判断内存是否与之前的值相同
//				Ext.MessageBox.show({
//					title : i18n._('errorNotice'),
//					msg : i18n._('Please adjust the size of the memory'),
//					buttons : Ext.MessageBox.OK,
//					icon : Ext.MessageBox.WARNING
//				});
//				return;
...
var resourceLimitPanel = Ext.create('Ext.form.Panel',{
	...
	items : [{
		...
		frame : true,
		layout : 'hbox',
		fieldLabel : i18n._('Bandwidth') + i18n._('(Mbps)'),
		margin : '25 0 0 0',
		labelAlign:'right',
		labelWidth : 100,
			...
			Ext.MessageBox.confirm(i18n._('Adjust'),i18n._('confirmResize'),function(btn){
				if(btn == 'yes'){
					...
					var adjust = Ext.Ajax.request({
						...
						jsonData: {
							'bwtIn' : BandwidthUp,
							'bwtOut' : BandwidthDown,
							'ipConnIn' : IPConnectionsUp,
							'ipConnOut' : IPConnectionsDown,
							'tcpConnIn' : TCPConnectionsUp,
							'tcpConnOut' : TCPConnectionsDown,
							'udpConnIn' : UDPConnectionsUp,
							'udpConnOut' : UDPConnectionsDown
						},
						success : function(form, action) {
							v_mask.hide();
							var obj = Ext.decode(form.responseText);
