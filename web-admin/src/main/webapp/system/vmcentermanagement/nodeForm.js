...
var nodeCPUCoreField = Ext.create('Ext.form.field.Number', {
	...
	labelAlign : 'right',
	//labelWidth : 10,
	//fieldLabel : i18n._('×'),//Core
	allowBlank : false,
	enforceMaxLength:true,
	maxLength : 20,	
	width : 50,//width : 150,
	minValue :1,	
	regex : /(^[0-9]{1,20}$)/,// 禁止输入空白字符/(^[0-9]{1,25}$)/
...
var submitButton = Ext.create('Ext.button.Button', {
	...
	handler : function() {
		...
		if(nodeCPUTypeField.getValue()==null ||nodeCPUTypeField.getValue()==''){
			Ext.MessageBox.show({
				title : i18n._('errorNotice'),
				msg : i18n._('please input') + i18n._('CPU'),
				buttons : Ext.MessageBox.OK,
				icon : Ext.MessageBox.WARNING
			});
			return;
		...
		//zone关联
		var zoneIdIndex = findZoneStore.find('name',nodeZoneField.getValue());
