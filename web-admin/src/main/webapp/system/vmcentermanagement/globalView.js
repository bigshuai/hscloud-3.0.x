...
//				var availableVmNum = globalViewStore.getAt(0).get('vmPaidNum')+globalViewStore.getAt(0).get('vmTrialNum');
				...
				stackChartData.push({
					...
					virtualCPUTip : globalViewStore.getAt(0).get(
							'virtualCPUApply'),
					virtualCPUUsed : globalViewStore.getAt(0).get(
							'virtualCPUApply'),
					virtualCPUTotal : (virtualCPUTotal1>0?virtualCPUTotal1:0),
					virtualMemoryTip : Math.ceil(globalViewStore.getAt(0).get(
							'virtualMemoryApply')/1024)+'G',
					virtualMemoryUsed : globalViewStore.getAt(0).get(
							'virtualMemoryApply')/1024,
					virtualMemoryTotal : (virtualMemoryTotal1>0?(virtualMemoryTotal1/1024):0),
					virtualDiskTip : globalViewStore.getAt(0).get(
							'virtualDiskApply')+'G',
					virtualDiskUsed : globalViewStore.getAt(0).get(
							'virtualDiskApply'),
				...
				stackChartData.push({
					...
					virtualMemoryTip : Math.ceil(globalViewStore.getAt(0).get('virtualMemoryUsed')/1024)+'G',
					virtualMemoryUsed : (globalViewStore.getAt(0).get('virtualMemoryUsed')/1024),
...
var chartCPU2 = Ext.create('Ext.chart.Chart', {
	animate : true,
	shadow :false,
	store : globalStackStore,// reStorePhyResource,storePhyResource
	width : 200,//180
	height : 125,
	theme : 'Category7',
	axes : [ {
