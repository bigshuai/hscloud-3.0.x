		...
		buttons:[
					{
						...
						handler : function() {
							...
							if(!vmRadioFlag){
								...
								if(timeUnit==1){
									vmRefreshPeriod = Ext.getCmp('numVMTime').getValue();
								}else{
									vmRefreshPeriod = Ext.getCmp('numVMTime').getValue()*60;
								}
...
var instanceModel = Ext.define('InstanceAdminVo', {
	...
	fields : [ 'vmId', 'vmName', 'remark','outComments','hostName', 'ipInner', 'ipOuter', 'cpuCore','memory', 'disk', 
	           'cpuUsage','memoryUsage', 'diskUsage', 'vmOS','vmStatus','hostAliases',
	           {name:'createTime',type: 'date', dateFormat: 'time'},'isEnable','userName','zoneCode',
	           'sysUser','iopsMonitorDetailBean','netMonitorDetailBean','diskMonitorDetailBean',
	           'cpuLimit','diskRead','diskWrite','bandWidthIn','bandWidthOut','ipConnectionIn',
	           'ipConnectionOut','tcpConnectionIn','tcpConnectionOut','udpConnectionIn','udpConnectionOut','processState'
	],//
	idProperty : 'vmId'
});
//虚拟机详情模板
...
var instanceStore = Ext.create('Ext.data.Store', {
	...
	proxy : new Ext.data.proxy.Ajax({
		url : path
				+ '/../monitoring/oss!findVmDetails.action?hostName='
	...
	listeners : {
		...
		load : function(instanceStore, records, successful, eOpts ){
			...
			lastRefreshTimeField.setValue(refreshDate.toLocaleTimeString());
		}
	}
});

//实时监控
var vmRealTimewindow = Ext.create('Ext.window.Window', {
	title : i18n._('onTime_monitor'),//实时监控
...
//虚拟备注Form
var RemarkFormPanel = Ext.create('Ext.form.Panel', {
	width: '440',
	height: '400',
	border:false,
	bodyPadding : 10,
...
buttons:[
         {
        	 ...
        	 handler:function(){
                 ...
                 var remark = editPanel.getComponent('rm_remark').getValue();
                 var outComments = editPanel.getComponent('rm_outComments').getValue();
                 var vmId = editPanel.getComponent('rm_vmId').getValue();
                 Ext.Ajax.request({  										
         			url : path+ '/../ops/ops!updateComments.action',
         			method : 'POST',
         			params : {				
         				'id' : vmId,
         				'comments' : remark,
         ...
         {text:i18n._('Cancel'), handler:function(){
        	 vmRefreshReset();//刷新设置重置
...
var connectParamFieldSet = Ext.create('Ext.form.FieldSet',
{
    xtype:'fieldset',
    border: false,
    padding:'0 0 0 0',
    ...
    items:[{
		  ...
		  listeners: {
			  focus: function(){
				  // 验证码输入框获取焦点时，更新验证码
			  	  ...
			  	  var value = '<img id="imageCode" alt="" title="看不清，换一个！" src="'+url+'">';
		  ...
		  handler: function(button) {
		 		remoteConnectionForm.getComponent(4).setValue("");
		        ...
		        if (codeField == null || codeField == "") {
               	...
               	Ext.Ajax.request({  										
            		url : path+ '/../ops/ops!getClientVNC.action',
            		method : 'POST',
            		params : {				
            			'id' : selectedVMUuid,
        		    	"code":Ext.getCmp("codeField").getValue()
            		},			
            		success: function(response, options){
            			var obj = Ext.decode(response.responseText);
            			if(!obj.success){
            				...
            				return;
            		 		...
            		 		var connectionParam = '<font style="color:green">远程连接: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' + obj.resultObject.proxyIP + ":" + obj.resultObject.proxyPort + "&nbsp;" + i18n._('vncTip4') + "</font>";
            		 		remoteConnectionForm.getComponent(4).setValue(connectionParam);
            		 	}		
            		},   
            	    failure:function(response, options){   
            	    	Ext.MessageBox.show({
...
//远程连接Form
var remoteConnectionForm=Ext.create('Ext.form.FormPanel', {
    width: '480',
    height: '220',
    border:false,
    autoScroll:true,
    ...
    items: [
       {
		...
		renderer: function(value){
        	return "<a href='../../download/vncviewer/tvnviewer.exe'>"+i18n._('downloadClient')+"</a>";
        }	
	...
	if(Ext.getCmp('password').isValid() && Ext.getCmp('confirmPassword').isValid()){
		if(Ext.getCmp('password').getValue()==Ext.getCmp('confirmPassword').getValue()){
	    	Ext.Ajax.request({
	    		...
	    		params: {
	    	    	"vmId":vmid,
	    	    	"password": Ext.getCmp('sysUser').getValue()+'/'+Ext.getCmp('password').getValue()
	    	    },
...
var passwordForm=Ext.create('Ext.form.FormPanel', {
    ...
    buttons:[
        {text:i18n._('OK'), handler:passwordSubmit},
        {text:i18n._('Cancel'), handler:function(){
        	vmRefreshReset();//刷新设置重置
        	passwordForm.getForm().reset();
        	passwordWin.hide();
...
var sellAction = Ext.create('Ext.Action', {
    ...
    handler: function(widget, event) {
        ...
        if (rec) {
        	...
        	window.location.href='hc_admin_vmPage.html?vmID='+rec.get('vmId');
...
var instanceGrid = Ext.create('Ext.grid.Panel',{
	autoWidth : true,
	store : instanceStore,
	//stateful : true,
	//frame : true,
		...
		items : [
		         {
					...
					handler : function() {						
						...
						vmCreateDiskArray = [];
						resetCreateVMForm();						
						createVMWin.setTitle(i18n._('CreateVM'));//创建虚拟机										
		         ...
		         },
				{
					...
					handler : function() {
						getSessionUser();
						var row = instanceGrid.getSelectionModel().getSelection();
						if (row.length == 0) {
							Ext.MessageBox.show({
								title : i18n._('notice'),
						...
						var record = instanceGrid.getStore().getById(id);										
						var status = row[0].get('vmStatus');
						if (status != 'noInstance') { 
							deleteVMWin.show();
						}										
						if (status == 'noInstance') {
							Ext.MessageBox.confirm(i18n._('submit'),i18n._('confirmTerminate'),function(btn) {
								if (btn == 'yes') {	
									//遮罩层
									var v_mask = new Ext.LoadMask(Ext
											.getBody(), {
					...
					xtype : 'button',
					text: '<font id="passwordFontId" color="#ffffff" >' + i18n._('password') + '</font>',//修改密码
					listeners: {
						 "mouseout" : function() {
							 document.getElementById("passwordFontId").style.color = "white";
						 },
						 "mouseover" : function() {
							 document.getElementById("passwordFontId").style.color = "black";
						 }
					...
					handler : function() {
						...
						if (row.length == 0) {
							...
							passwordForm.getForm().reset();
							Ext.getCmp('sysUser').setValue(sysUser); 
							passwordWin.show();
						}
				...
				{
					xtype : 'button',									
					text: '<font id="vmstart" color="#ffffff" >' + i18n._('start') + '</font>',//启动
					listeners: {
						 "mouseout" : function() {
					...
					handler : function() {	
						...
						if (status == 'SUSPENDED') {											
							var start = Ext.Ajax.request({
										url : path+ '/../ops/ops!startVM.action',
										method : 'POST',
										params : 'id=' + vmid,
										success : function(form, action) {
											var obj = Ext.decode(form.responseText);											
				...
				{
					xtype : 'button',									
					text: '<font id="vmreboot" color="#ffffff" >' + i18n._('REBOOT') + '</font>',//重启
					listeners: {
						 "mouseout" : function() {
					...
					handler : function() {
						...
						if (row.length == 0) {
							...
							if (status == 'ACTIVE' || status == 'SUSPENDING' || status == 'SHUTOFF') {
								Ext.MessageBox.confirm(i18n._('submit'),i18n._('confirmReboot'),function(btn) {
									if (btn == 'yes') {																		
										var start = Ext.Ajax.request({
											url : path+ '/../ops/ops!rebootVM.action',
											method : 'POST',
											params : 'id='+ vmid,
											success : function(form,action) {
												var obj = Ext.decode(form.responseText);												
				...
				{
					xtype : 'button',									
					text: '<font id="vmclose" color="#ffffff" >' + i18n._('CLOSE') + '</font>',//关闭
					listeners: {
						 "mouseout" : function() {
					...
					handler : function() {
						...
						if (row.length == 0) {
							...
							if (status == 'ACTIVE') {
								Ext.MessageBox.confirm(i18n._('submit'),i18n._('confirmClose'),function(btn) {
									if (btn == 'yes') {																		
										var start = Ext.Ajax.request({
											url : path+ '/../ops/ops!closeVM.action',
											method : 'POST',
											params : 'id='+ vmid,
											success : function(form,action) {
												var obj = Ext.decode(form.responseText);
				...
				},
				{
					xtype : 'button',	
					hidden:true,
					text: '<font id="osRepair" color="#ffffff" >' + i18n._('osRepair') + '</font>',//系统修复,主机修复
					listeners: {
						 "mouseout" : function() {
					...
					handler : function() {
						...
						if (row.length == 0) {
							...
							if (status == 'ACTIVE') {
								Ext.MessageBox.confirm(i18n._('submit'),i18n._('confirmOSRepair'),function(btn) {
									if (btn == 'yes') {																		
										var start = Ext.Ajax.request({
											url : path+ '/../ops/ops!repairOS.action',
											method : 'POST',
											params : 'id='+ vmid,
											success : function(form,action) {
												var obj = Ext.decode(form.responseText);
				...
				{
					xtype : 'button',									
					text: '<font id="vmmigrate" color="#ffffff" >' + i18n._('Migrate') + '</font>',//迁移
					listeners: {
						 "mouseout" : function() {
							 document.getElementById("vmmigrate").style.color = "white";
						 },
						 "mouseover" : function() {
							 document.getElementById("vmmigrate").style.color = "black";
						 }
					...
					handler : function() {
						...
						var status=record.get('vmStatus');
						var zoneCode=record.get('zoneCode');
						...
						if(status=='ACTIVE'){
							...
							vmCPUUsed.setValue(record.get('cpuCore')*record.get('cpuUsage')/100);
							vmMemoryUsed.setValue(record.get('memory')*record.get('memoryUsage')/100);
							vmDiskUsed.setValue(record.get('disk')*record.get('diskUsage')/100);
				...
				{
					xtype : 'button',									
					text: '<font id="vmadjust" color="#ffffff" >' + i18n._('Adjust') + '</font>',//调整
					listeners: {
						 "mouseout" : function() {
					...
					handler : function() {
						...
						var vmZoneCode=new String(row[0].get('zoneCode'));
						var arrayOfStrings = vmZoneCode.split("$");
						var bandWidthIn = (row[0].get('bandWidthIn')>0?row[0].get('bandWidthIn'):1);
						var bandWidthOut = (row[0].get('bandWidthOut')>0?row[0].get('bandWidthOut'):1);
						var ipConnectionIn = (row[0].get('ipConnectionIn')>0?row[0].get('ipConnectionIn'):2500);
						var ipConnectionOut = (row[0].get('ipConnectionOut')>0?row[0].get('ipConnectionOut'):6000);
						var tcpConnectionIn = (row[0].get('tcpConnectionIn')>0?row[0].get('tcpConnectionIn'):2000);
						var tcpConnectionOut = (row[0].get('tcpConnectionOut')>0?row[0].get('tcpConnectionOut'):4000);
						var udpConnectionIn = (row[0].get('udpConnectionIn')>0?row[0].get('udpConnectionIn'):500);
						var udpConnectionOut = (row[0].get('udpConnectionOut')>0?row[0].get('udpConnectionOut'):2000);
						...
						for(var i=0;i<vmIpData.length;i++){
							if(vmIpData[i]!=""){
								var IpShowField = Ext.create('Ext.form.field.Text', {
									id:'ipShowField'+vmIpData[i],
									width : 100,
									margin:'0 0 0 85',
									editable : false,
									readOnly : true
								});
								IpShowField.setValue(vmIpData[i]);
								var vmIPDeleteBtn = Ext.create('Ext.Button', {
									itemId:vmIpData[i],
									text : '×',//删除IP
									handler : function() {
										var delIP=this.getItemId();
										Ext.MessageBox.confirm(i18n._('submit'),i18n._('Are you sure to delete'),function(btn) {
											if (btn == 'yes') {
												...
												var updateVmIP = Ext.Ajax.request({
													url : path + '/../ops/ops!deleteIPOfVm.action',
													method : 'POST',
													params : {						
														'id' : id,
														'oldIP' : delIP
											...
											createVMWin.remove(adjustVMForm,false);
											createVMWin.hide();
											v_mask.show();
										});
...
//						alert(configAdjustPanel.query('fieldcontainer').length);//
						for(var i=configAdjustPanel.query('fieldcontainer').length;i>6;i--){							
							//alert(configAdjustPanel.query('fieldcontainer')[i-1].is('fieldcontainer'));
							configAdjustPanel.query('fieldcontainer')[i-1].removeAll(true);
						}	
...
//							vmIPAdjustField.setValue(vmIpOuter);
							...
							vmAdjustCPUCoreField.setValue(vmCPUCore);
							resourceLimitBandwidthUp.setValue(bandWidthIn);
							resourceLimitBandwidthDown.setValue(bandWidthOut);
							resourceLimitIPConnectionsUp.setValue(ipConnectionIn);
							resourceLimitIPConnectionsDown.setValue(ipConnectionOut);
							resourceLimitTCPConnectionsUp.setValue(tcpConnectionIn);
							resourceLimitTCPConnectionsDown.setValue(tcpConnectionOut);
							resourceLimitUDPConnectionsUp.setValue(udpConnectionIn);
							resourceLimitUDPConnectionsDown.setValue(udpConnectionOut);
							//alert('vmlist OldIp:'+vmOldIPAdjustField.getValue());
							createVMWin.add(adjustVMForm);// adjustTab
							createVMWin.show();
						}else{
							Ext.MessageBox.show({
								title : i18n._('notice'),
				...
				'-',
				{
					...
					text: '<font id="vmmonitor" color="#ffffff" >' + i18n._('monitor') + '</font>',//监控
					listeners: {
						 "mouseout" : function() {
							 document.getElementById("vmmonitor").style.color = "white";
						 },
						 "mouseover" : function() {
							 document.getElementById("vmmonitor").style.color = "black";
						 }
					...
					handler : function() {	
						...
						if (row.length == 0) {
							...
							if(status=='ACTIVE'){												
								...
								proxy = realTimeVMStore.getProxy();
								proxy.setExtraParam('vmId',vmid) ;												
								realTimeVMStore.load();												
								vmRealTimewindow.show();
							}
							if (status != 'ACTIVE') {
								Ext.MessageBox.show({
									title : i18n._('notice'),
				...
				{
					xtype : 'button',									
					text: '<font id="vmbackups" color="#ffffff" >' + i18n._('backups') + '</font>',//备份
					listeners: {
						 "mouseout" : function() {
							 document.getElementById("vmbackups").style.color = "white";
						 },
						 "mouseover" : function() {
							 document.getElementById("vmbackups").style.color = "black";
						 }
					...
					handler : function() {
						...
						if (row.length == 0) {
							...
							if (status == 'ACTIVE') {
								Ext.MessageBox.confirm(i18n._('submit'),i18n._('confirmBackups'),function(btn) {
									if (btn == 'yes') {
										var vmBak = Ext.Ajax.request({
											url : path+ '/../ops/ops!backupsVm.action',
											method : 'POST',
											params : 'id='+ vmid,
											success : function(form,action) {
												var obj = Ext.decode(form.responseText);
				...
				{
					xtype : 'button',									
					text: '<font id="vmrenew" color="#ffffff" >' + i18n._('renew') + '</font>',//还原
					listeners: {
						 "mouseout" : function() {
							 document.getElementById("vmbackups").style.color = "white";
						 },
					...
					handler : function() {
						...
						if (row.length == 0) {
							...
							if (status == 'ACTIVE') {
								Ext.MessageBox.confirm(i18n._('submit'),i18n._('confirmRenew'),function(btn) {
									if (btn == 'yes') {
										var vmRollBack = Ext.Ajax.request({
											url : path+ '/../ops/ops!renewVm.action',
											method : 'POST',
											params : 'id='+ vmid,
											success : function(form,action) {
												var obj = Ext.decode(form.responseText);
				...
				{
					xtype : 'button',									
					text: '<font id="vmremote" color="#ffffff" >' + i18n._('remote') + '</font>',//远程访问
					listeners: {
						 "mouseout" : function() {
								...
								Ext.Ajax.request({
									...
									success : function(form,action) {
										...
										if (!obj.success) {
											...
											// 在原来的提示界面显示VNC连接失败
											remoteConnectionForm.getComponent(0).setValue('<br/><br/><font color="red" style="font-weight: bold;margin-left: 110px;">VNC远程连接失败，您可以尝试以下方式连接</font><br/><br/>');
											return;
										} else {
											remoteConnectionForm.getComponent(0).setValue('<img id="enterVNC" src="images/EnterVNC.jpg" style="margin-left: 170px;margin-top:25px;margin-bottom:15px;cursor:pointer;"/>');
											// 确定进入VNC
...
//					disabled:true,
				...
				{
					xtype : 'button',									
					text: '<font id="vmremark" color="#ffffff" >' + i18n._('remark') + '</font>',//备注  这里说明下因为国际化里备注是remark，数据库中是comments所以在取这个属性时是jiangcomments转成了remark但是在修改时发现action的文件中有remark重名问题，就改为使用comments了。
					listeners: {
						 "mouseout" : function() {
					...
					handler : function() {
				        ...
				        if(null != row[0]) {
				            ...
				            //var editPanel = RemarkFormPanel;
				            Ext.getCmp('rm_remark').setValue(row[0].get('remark'));
				            Ext.getCmp('rm_outComments').setValue(row[0].get('outComments'));
				            Ext.getCmp('rm_vmId').setValue(row[0].get('vmId'));
				            remarkWin.show();
...
//				                        if(editPanel.form.isValid()) {
//				                            var data = row[0];
//				                            data.remark = editPanel.getComponent('rm_remark').getValue();
//				                            data.outComments = editPanel.getComponent('rm_outComments').getValue();
//				                            data.vmId = editPanel.getComponent('rm_vmId').getValue();
//				                            Ext.Ajax.request({  										
//				                    			url : path+ '/../ops/ops!updateComments.action',
//				                    			method : 'POST',
//				                    			params : {				
						...
						if (row.length == 0) {
							...
							if (isEnable != 0) {
								Ext.MessageBox.confirm(i18n._('submit'),i18n._('confirmEnableSC'),function(btn) {
									if (btn == 'yes') {																		
										var start = Ext.Ajax.request({
											url : path+ '/../ops/ops!activeVM.action',
											method : 'POST',
											params : 'id='+ vmid,
											success : function(form,action) {
												var obj = Ext.decode(form.responseText);
						...
						if (row.length == 0) {
							...
							if (isEnable == 0) {
								Ext.MessageBox.confirm(i18n._('submit'),i18n._('confirmDisableSC'),function(btn) {
									if (btn == 'yes') {																		
										var start = Ext.Ajax.request({
											url : path+ '/../ops/ops!freezeVM.action',
											method : 'POST',
											params : 'id='+ vmid,
											success : function(form,action) {
												var obj = Ext.decode(form.responseText);												
...
//				},
				{
                    xtype:'splitbutton',
                    id:'vmSplitbutton',
                    text:'<font color="#ffffff" id="vmSplit">' + i18n._('QuickSearch')+':' + '</font>', //快速查询
                    listeners: {
						 "mouseout" : function() {
                        ...
                        items: [
                            {
                                text: i18n._('vm_name'),
                                handler: function(){
                                	Ext.getCmp('vmSplitbutton').setText('<font color="#ffffff" id="vmSplit">' + i18n._('vm_name') + '</font>');
                            ...
                            },                            
                            {
                            	text: i18n._('ipOuter'),
                            	handler: function(){
                            		Ext.getCmp('vmSplitbutton').setText('<font color="#ffffff" id="vmSplit">' + i18n._('ipOuter') + '</font>');
                            		var instanceStoreProxy=instanceStore.getProxy();
                            ...
                            },
                            {
                                text: i18n._('ipInner'),
                                handler: function(){
                                    Ext.getCmp('vmSplitbutton').setText('<font color="#ffffff" id="vmSplit">' + i18n._('ipInner') + '</font>');
                                    var instanceStoreProxy=instanceStore.getProxy();
                            ...
                            /*{
                                text: i18n._('host_name'),
                                disabled:menuDisabled,
                                handler: function(){
                                	Ext.getCmp('vmSplitbutton').setText('<font color="#ffffff" id="vmSplit">' + i18n._('host_name') + '</font>');
                                	var instanceStoreProxy=instanceStore.getProxy();
                            ...
                            },*/
                            {
                                text: i18n._('Owner'),
                                handler: function(){
                                    Ext.getCmp('vmSplitbutton').setText('<font color="#ffffff" id="vmSplit">' + i18n._('Owner') + '</font>');
                                    var instanceStoreProxy=instanceStore.getProxy();
                                    instanceStoreProxy.setExtraParam('type','vmOwner');
                                    checkFlag=true;
                                }
                            }, 
                            {
                                ...
                                handler: function(){
                                	Ext.getCmp('vmSplitbutton').setText('<font color="#ffffff" id="vmSplit">' + i18n._('inner_remark') + '</font>');
                                	var instanceStoreProxy=instanceStore.getProxy();
                                	instanceStoreProxy.setExtraParam('type','remark');  
                                	checkFlag=true;
                                }
                            }, 
                            {
                                ...
                                handler: function(){
                                	Ext.getCmp('vmSplitbutton').setText('<font color="#ffffff" id="vmSplit">' + i18n._('outComments') + '</font>');
                                	var instanceStoreProxy=instanceStore.getProxy();
                                	instanceStoreProxy.setExtraParam('type','outComments');  
                                	checkFlag=true;
                                }
                            }
                         ]
                ...
                }, 
				{
					labelWidth : 50,
					xtype : 'searchfield',
					tooltip:i18n._('vm_name')+'/'+i18n._('host_name')+'/'+i18n._('IPAddress'),
...
//			var string2=string1+'</br>'+string.substring(20);
			metadata.tdAttr = 'data-qtip="' + string + '"';
			//metadata.style = 'margin-left:0px;word-break: break-all; word-wrap:break-word;';
		    return data;							
		}
	}, {
		text : i18n._('node'),
		...
		renderer :function(data, metadata, record, rowIndex, columnIndex, store){
			...
			renderText = "";
			var index1 = Math.floor(vmShowIpData.length / 8)
//			alert("index1: "+index1);
...
//			alert("index2: "+index2);
			if (index1 > 0){
				...
				renderText=renderText.substring(renderText.length-1,0);
			}else{
				renderText = ip;
			}
	...
	},
	{
		...
		renderer : function(value,metadata, record, rowIndex, columnIndex, store){
			var text='';
			var status = record.get('vmStatus');
			if(status == "noInstance" || status == "ERROR"){
				value=3;
			}
			switch(value){
		...
		renderer : renderDescnMemory
	}, {
		header : i18n._('IOPS')+'(KB/sec)',
		dataIndex : 'iopsMonitorDetailBean',
		flex : 0.7,
		sortable: true,
		renderer : renderDescnIOPS
	}, {
		header : i18n._('NET')+'(KBps)',
		dataIndex : 'netMonitorDetailBean',
		flex : 0.7,
		sortable: true,
		renderer : renderDescnNET
	}, {
		text : i18n._('DISK')+'(G)',
		flex : 0.4,
//		hidden :true,
...
//		renderer:record.get('remark')
	...
	listeners:{
		itemdblclick :function(instanceGrid,record, item, index, e, eOpts){							
...
//			createVMWin.show();							
		...
		itemclick:function(instanceGrid,record, item, index, e, eOpts){
			vmRunner.stopAll();
		},
...
function renderDescnVmStatus(value, cellmeta, record, rowIndex, columnIndex, store) {
		...
		/*else if(status == "VERIFY_RESIZE"){//确认迁移
...
function renderDescnCPU(value, cellmeta, record, rowIndex, columnIndex, store) {
	...
	if(cpuMonitorDetail!=null){
		...
		workloadLimit=cpuMonitorDetail.workloadLimit;
	}
	if(load_avg != null){
...
function renderDescnDisk(value, cellmeta, record, rowIndex, columnIndex, store) {
	...
	if(singleDetailBeanList != null){
		for(var i=0;i<singleDetailBeanList.length;i++){
			diskList+=singleDetailBeanList[i].title+':'+Ext.util.Format.round(singleDetailBeanList[i].usage,1)+'%'+'<br/>';
		}
	}	
	if(status>100){
		status=100;
...
function renderDescnIOPS(value, cellmeta, record, rowIndex, columnIndex, store) {
	...
	if(iopsMonitorDetail!=null){
		...
		readActual=(iopsMonitorDetail.readActual==undefined?0:iopsMonitorDetail.readActual);
		writeActual=(iopsMonitorDetail.writeActual)==undefined?0:iopsMonitorDetail.writeActual;
		readLimit = (iopsMonitorDetail.readLimit==undefined?1:iopsMonitorDetail.readLimit);
		writeLimit = (iopsMonitorDetail.writeLimit==undefined?1:iopsMonitorDetail.writeLimit);
	}
//	alert(readLimit>0);
	if(readLimit>0){		
	...
	if(singleDetailBeanList != null){
		for(var i=0;i<singleDetailBeanList.length;i++){
			iopsList+=singleDetailBeanList[i].title+':'+'read '+Ext.util.Format.round(singleDetailBeanList[i].iopsRead,1)+'KB/sec,'				
					+'write '+Ext.util.Format.round(singleDetailBeanList[i].iopsWrite,1)+'KB/sec'+'<br/>';
		}
	}	
	if(readStatus>100){
		readStatus=100;
...
function renderDescnNET(value, cellmeta, record, rowIndex, columnIndex, store) {
	...
	if(netMonitorDetail!=null){
		...
		readActual=(netMonitorDetail.readActual==undefined?0:netMonitorDetail.readActual);
		writeActual=(netMonitorDetail.writeActual==undefined?0:netMonitorDetail.writeActual);
		readLimit = (netMonitorDetail.readLimit==undefined?1:netMonitorDetail.readLimit);
		writeLimit = (netMonitorDetail.writeLimit==undefined?1:netMonitorDetail.writeLimit);
	}
	if(readLimit>0){
		readStatus = Ext.util.Format.round(readActual/readLimit*100,1);
	}
