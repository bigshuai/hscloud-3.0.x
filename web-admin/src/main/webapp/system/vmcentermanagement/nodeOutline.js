...
//				});
				...
				if(successful && nodeViewStore.getCount()>0){
					...
					CPUValueLabel.update({html:'<div class="TipDiv" data-qtitle="" data-qtip="'+cpuType+'GHz×'+cpuCore+i18n._('core')+'">'+cpuType+'GHz×'+cpuCore+i18n._('core')+'</div>'});	
					...
					barChartData.push({
						virtualCPUTip : nodeViewStore.getAt(0).get(
								'virtualCPUApply')+'core',
						virtualCPUUsed : nodeViewStore.getAt(0).get(
								'virtualCPUApply'),										
						virtualCPUTotal : (virtualCPUTotal1>0?virtualCPUTotal1:0),
						virtualMemoryTip : Math.ceil(nodeViewStore.getAt(0).get(
								'virtualMemoryApply')/1024)+'G',
						virtualMemoryUsed : nodeViewStore.getAt(0).get(
								'virtualMemoryApply')/1024,
						virtualMemoryTotal : (virtualMemoryTotal1>0?Math.ceil(virtualMemoryTotal1/1024):0),
						virtualDiskTip : nodeViewStore.getAt(0).get(
								'virtualDiskApply')+'G',
						virtualDiskUsed : nodeViewStore.getAt(0).get(
								'virtualDiskApply'),
					...
					barChartData.push({
						...
						virtualMemoryTip : Math.ceil(nodeViewStore.getAt(0).get(
								'virtualMemoryUsed')/1024)+'G',
						virtualMemoryUsed : nodeViewStore.getAt(0).get(
								'virtualMemoryUsed')/1024,
						virtualMemoryTotal : (virtualMemoryTotal2>0?Math.ceil(virtualMemoryTotal2/1024):0),
						virtualDiskTip : nodeViewStore.getAt(0).get(
								'virtualDiskUsed')+'G',
						virtualDiskUsed : nodeViewStore.getAt(0).get(
								'virtualDiskUsed'),
