var IMGFOLDERPATH = rootPath+"/skin/"+customerPath+"/js/common/dialog/images/";//图片路径配置
...
if (window.HTMLElement) {//给FF添加IE专有的属性和方法
	if (navigator.userAgent.indexOf("Firefox") > 0) {//解决IE下面不支持__defineGetter__语法问题
...
function Dialog(strID) {
	...
	this.ID = strID;//new一个Dialog的ID
	this.isModal = true;
	this.Width = 490;//Dialog的宽度
	this.Height = 100;//Dialog的高度
	this.Top = 0;
	this.Left = 0;
	this.ParentWindow = null;
	this.onLoad = null;
	this.Window = null;
	this.Title = i18n.get("systemPrompt");//Dialog的默认标题为系统提示
	this.URL = null;//Dialog引用的网站内容
	this.innerHTML = null;//Dialog里面显示的html
...
Dialog.prototype.show = function() {
	...
	if (this.Top == 0) {
		//firefox下面有问题,把st设置为0   原版本为://this.Top = (ch - this.Height - 30) / 2 + st - 8;
		this.Top = (ch - this.Height - 30) / 2 + 0 - 8;
	}
	if (this.Left == 0) {
	...
	var arr = [];
	//禁止选中dialog的文字和复制   style='-moz-user-select:none;' oncontextmenu='stopEvent(event);' onselectstart='stopEvent(event);'
	...
	arr.push("<table border='0' cellpadding='0' cellspacing='0' width='"+(this.Width+26)+"'>");
	...
	arr.push("    <td align='center' valign='top'><a href='#;' id='_forTab_"+this.ID+"' style='margin: 0;'></a>");
	...
	arr.push("            <div id='_DialogButtons_"+this.ID+"' style='text-align:right; border-top:#dadee5 1px solid; padding:8px 20px;'>");
	arr.push("           	<input class='dialogButton1' id='_ButtonOK_"+this.ID+"'  type='button' value='" + i18n.get("OK") + "'>");
	arr.push("           	<input class='dialogButton2' id='_ButtonCancel_"+this.ID+"'  type='button' onclick=\"Dialog.getInstance('"+this.ID+"').close();\" value='" + i18n.get("Cancel") + "'>");
	arr.push("            </div></td></tr>");
	arr.push("      </table><a href='#;' onfocus='$_(\"_forTab_"+this.ID+"\").focus();' style='margin: 0;'></a></td>");
	...
	var div = pw.$_("_DialogDiv_" + this.ID);
	if (!div) {
	...
	if (this.innerElementId) {
		var innerElement = $_(this.innerElementId);
		...
		if (isIE) {
			...
			pw.$_("_Covering_" + this.ID).parentNode.appendChild(fragment);
		} 
		else {
			pw.$_("_Covering_" + this.ID).parentNode.appendChild(innerElement);
		}
	}
	pw.$_("_DialogDiv_" + this.ID).DialogInstance = this;
	if (this.URL) {
		pw.$_("_DialogFrame_" + this.ID).DialogInstance = this;
	}
	pw.Drag.init(pw.$_("_draghandle_" + this.ID), pw.$_("_DialogDiv_" + this.ID));// 注册拖拽方法
	if (!isIE) {
		pw.$_("_DialogDiv_" + this.ID).dialogId = this.ID;
		pw.$_("_DialogDiv_" + this.ID).onDragStart = function() {
			$_("_Covering_" + this.dialogId).style.display = "";
		}
		pw.$_("_DialogDiv_" + this.ID).onDragEnd = function() {
			$_("_Covering_" + this.dialogId).style.display = "none";
		}
	}
	this.OKButton = pw.$_("_ButtonOK_"+this.ID);
	this.CancelButton = pw.$_("_ButtonCancel_"+this.ID);
	if (this.ShowMessageRow) {//显示标题图片
		$E.show(pw.$_("_MessageRow_" + this.ID));
		if (this.MessageTitle) {
			pw.$_("_MessageTitle_" + this.ID).innerHTML = this.MessageTitle;
		}
		if (this.Message) {
			pw.$_("_Message_" + this.ID).innerHTML = this.Message;
		}
	}
	if (!this.ShowButtonRow) {//显示按钮栏
		pw.$_("_ButtonRow_" + this.ID).hide();
	}
	if (this.CancelEvent) {
...
Dialog.prototype.close = function() {
	if (this.innerElementId) {
		var innerElement = $E.getTopLevelWindow().$_(this.innerElementId);
		innerElement.style.display = "none";
		if (isIE) {//ie下不能跨窗口拷贝元素
	...
	if (this.WindowFlag) {
		...
		if (this.AlertFlag) {
			$E.hide(pw.$_("_AlertBGDiv"));
		}
		if (!flag && !this.AlertFlag) {//此处是为处理弹出窗口被关闭后iframe立即被重定向时背景层不消失的问题
			pw.eval('window._OpacityFunc = function(){var w = $E.getTopLevelWindow();$E.hide(w.$_("_DialogBGDiv"));}');
			pw._OpacityFunc();
		}
...
Dialog.prototype.addButton = function(id, txt, func) {
	...
	var pw = $E.getTopLevelWindow();
	pw.$_("_DialogButtons_"+this.ID).$T("input")[0].getParent("a").insertAdjacentHTML("beforeBegin", html);
	pw.$_("_Button_"+this.ID+"_"+id).onclick = func;
}
...
Dialog.alert = function(msg, func, w, h) {
	...
	diag.Width = w ? w : 490;
	diag.Height = h ? h : 100;
	diag.Title = i18n.get("systemPrompt");
	...
	diag.show();
	pw.$_("_AlertBGDiv").style.display="";
	$E.hide(pw.$_("_ButtonOK_"+diag.ID));
	var win = pw.$_("_DialogFrame_"+diag.ID).contentWindow;
	var doc = win.document;
	doc.open();
	doc.write("<body oncontextmenu='return false;'></body>") ;
	var arr = [];
	arr.push("<table height='100%' border='0' cellpadding='10' cellspacing='0'>");
	arr.push("<tr><td width='60' align='right'><img id='Icon' src='"+IMGFOLDERPATH+"icon_alert.png' width='44' height='44' align='absmiddle'></td>");
	arr.push("<td align='left' id='Message' style='font-family:Microsoft Yahei,verdana;font-size: 14px;font-weight: bold;color: #555555;'>"+msg+"</td></tr></table>");
	...
	diag.CancelButton.value = i18n.get("OK");
	diag.CancelButton.style.background = "none repeat scroll 0 0 #EAEAEA";
	diag.CancelButton.style.border = "1px solid #3399FF";
	diag.CancelButton.focus();
	pw.$_("_DialogButtons_" + diag.ID).style.textAlign = "right";
}
Dialog.confirm = function(msg, func1, func2, w, h) {
	...
	diag.Width = w ? w : 490;
	diag.Height = h ? h : 100;
	diag.Title = i18n.get("infoConfirm");
	diag.URL = "javascript:void(0);";
	diag.AlertFlag = true;
	diag.CancelEvent = function() {
		diag.close();
	...
	diag.show();
	pw.$_("_AlertBGDiv").style.dispaly="";
	var win = pw.$_("_DialogFrame_"+diag.ID).contentWindow;
	var doc = win.document;
	doc.open();
	doc.write("<body oncontextmenu='return false;'></body>") ;
	...
	if (d) {
		try {
			...
			if (!this.AlertFlag) {
				...
				if (!flag) {
					$E.getTopLevelWindow().$_("_DialogBGDiv").style.opacity = "0.4";
					$E.getTopLevelWindow().$_("_DialogBGDiv").style.filter = "alpha(opacity=40)";
				}
			}
			if (d.AlertFlag) {
				$E.show($E.getTopLevelWindow().$_("_AlertBGDiv"));
			}
...
Dialog.setPosition = function() {
	...
	for (i = 0; i < DialogArr.length; i++) {
		pw.$_("_DialogDiv_" + DialogArr[i]).DialogInstance.setPosition();
	}
}
Dialog.prototype.setPosition=function(){
	...
	pw.$_(this.bgdivID).style.height= sh + "px";
}
var Drag={
    "obj":null,
	...
	"drag":function(e){
		...
		var top=parseInt(handle.root.style.top);
		var left=parseInt(handle.root.style.left);
		if (isIE) {//作用是将所有鼠标事件捕获到handle对象，对于firefox，以用preventDefault来取消事件的默认动作：
			Drag.obj.setCapture();
		}
		else {
			e.preventDefault();
...
/**
 * 1.直接alert非html文字提示信息:              openDialog("文字内容");
 * 2.弹出包含html的dialog,不需要第二层dialog:   openDialog("html内容", "标题", 400, 600);
 * 3.弹出confirm确认提示文字的dialog,点击确定弹出执行结果dialog:
	 Dialog.confirm("确定......吗？", function() {
		 //点击确定按钮执行的代码和逻辑
		 openDialog("//点击确定按钮执行的代码");
	 });
 * 4.弹出包含html的dialog,点击确定按钮弹出第二层dialog:
	 var dialog = new Dialog("Dialog");
	 dialog.Title = "标题";
	 dialog.innerHTML = "<table><tr><td>11111111111111</td></tr></table>";
	 dialog.Height = 80;
	 dialog.Width = 500;
	 dialog.OKEvent = function() {//点击确定按钮调用的方法
	     //点击确定按钮代码的逻辑，在恰当的时候调用dialog.close();来关闭第一个dialog
		 dialog.close();//关闭第一个dialog
	 };
	 dialog.show();
 */
function openDialog(contentParam, titleParam, heightParam, widthParam) {
	if ((contentParam.indexOf("</table>") == -1 && contentParam.indexOf("</tr>") == -1
			&& contentParam.indexOf("</td>") == -1 && contentParam.indexOf("</div>") == -1 
			&& contentParam.indexOf("</iframe>") == -1) && (!titleParam && !heightParam 
			&& !widthParam)) {//1.直接alert非html文字信息
		Dialog.alert(contentParam);
	}
	else if ((contentParam.indexOf("</table>") > 0 || contentParam.indexOf("</tr>") > 0
			|| contentParam.indexOf("</td>") > 0 || contentParam.indexOf("</div>") > 0 
		    || contentParam.indexOf("</iframe>") > 0) && (titleParam && heightParam
			&& widthParam)) {//2.弹出包含html静态页面的dialog,不需要弹出第二层dialog
		var dialog = new Dialog("Dialog");
		dialog.Title = titleParam;
		dialog.innerHTML = contentParam;
		dialog.Height = heightParam;
		dialog.Width = widthParam;
